...
exports.run = async (client, message, args) => {
  ...
  .setColor('#8c0046') 
    ...
    var canal = client.channels.cache.get('747202301168582686'); // Puxando o canal aonde iremos enviar que o usuário tomou um warn 
  // Requisitando a permissão de Administrador
    ...
    if(warnstaff[membro.id].warnstaff == 4) {
      membro.roles.remove("744737156471455785")
    ...
    if(warnstaff[membro.id].warnstaff == 5) {
      let embedi = new Discord.MessageEmbed()
        ...
        .setFooter(`Você foi banido do OmegaMC pelo Staff: ${message.author.username}, Tenha um bom dia!`)
      
      await membro.send(embedi)
      message.guild.member(membro).ban(motivo);
      membro.send('Você foi banido do OmegaMC, Tenha um bom dia!')
